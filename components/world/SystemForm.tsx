...
import { createWorldSystem, updateWorldSystem } from "@/lib/actions";
...
export function SystemForm({ worldId, worldName, storyId, stories = [], entry, onSave, onCancel }: SystemFormProps) {
    const [name, setName] = useState(entry?.name || "");
    const [category, setCategory] = useState(entry?.category || "Magic");
    ...
    useEffect(() => {
        if (entry) {
            setName(entry.name || "");
            setCategory(entry.category || "Magic");
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isEdit) {
                await updateWorldSystem(entry.id, data);
            } else {
                await createWorldSystem(worldId, data);
            }
            onSave();
        } catch (error) {
    ...
    return (
        <Card className="max-w-5xl mx-auto">
            <CardHeader>
                <CardTitle>{isEdit ? "Edit System" : "Create System"}</CardTitle>
                <CardDescription>
                    Document a system (e.g. Magic, Laws, Technology, Economy) in your world.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="lg:col-span-7 flex flex-col justify-between gap-6">
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <Label htmlFor="name" className="text-xs font-bold uppercase tracking-wider text-muted-foreground">System Name</Label>
                                    <Input
                                        id="name"
                                        placeholder="e.g. Elemental Magic System"
                                        value={name}
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            id="category"
                                            placeholder="e.g. Magic, Technology, Laws"
                                            value={category}
                    ...
                    <div className="space-y-2 pt-2">
                        ...
                        <Textarea
                            id="description"
                            placeholder="A brief summary or detailed mechanics of this system..."
                            value={description}
                    ...
                    <div className="flex items-center justify-end gap-3 pt-6 border-t">
                        ...
                        >
                            ...
                            {isEdit ? "Update System" : "Save System"}
                        </Button>
                    </div>
                </form>
            </CardContent>
